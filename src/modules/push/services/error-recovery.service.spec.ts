...
describe('ErrorRecoveryService', () => {
  ...
  describe('executeWithRetry', () => {
    ...
    it('should not retry on non-retryable errors', async () => {
      const operation = vi
        .fn()
        .mockRejectedValue(
          new PushError(PushErrorCode.VAL_INVALID_FILE, 'Invalid file'),
        );
    ...
    it('should calculate exponential backoff correctly', async () => {
      ...
      const operation = vi
        .fn()
        .mockRejectedValueOnce(
          new PushError(PushErrorCode.NET_TIMEOUT, 'Timeout'),
        )
        .mockRejectedValueOnce(
          new PushError(PushErrorCode.NET_TIMEOUT, 'Timeout'),
        )
  ...
  describe('generateRecoverySuggestion', () => {
    ...
    it('should return custom suggestion for specific errors', () => {
      ...
      const suggestion = service.generateRecoverySuggestion(
        errorWithoutRemediation,
      );
  ...
  describe('logErrorDetails', () => {
    ...
    it('should include stack trace when requested', () => {
      const logSpy = vi
        ...
        .mockImplementation(() => {});
      const error = new PushError(
        PushErrorCode.SYS_INTERNAL_ERROR,
        'Internal error',
      );
