...
describe('SecureStorageService', () => {
  ...
  describe('storeCredential', () => {
    it('should store encrypted credential', async () => {
      await service.storeCredential('test-namespace', 'api-key', 'secret-value', true);
    ...
    it('should store unencrypted credential', async () => {
      await service.storeCredential('test-namespace', 'config', 'plain-value', false);
    ...
    it('should store credential with TTL', async () => {
      await service.storeCredential('test-namespace', 'token', 'temp-value', true, 60);
  ...
  describe('getCredential', () => {
    ...
    it('should return null for expired credential', async () => {
      ...
      await service.storeCredential('test-namespace', 'token', 'temp-value', true, 0.001);
  ...
  describe('deleteCredential', () => {
    ...
    it('should handle deleting non-existent credential', async () => {
      await expect(service.deleteCredential('test-namespace', 'missing')).resolves.not.toThrow();
    });

    it('should delete file when last credential is removed', async () => {
      await service.storeCredential('test-namespace', 'only-key', 'value', false);
  ...
  describe('cleanupExpired', () => {
    it('should clean up expired credentials', async () => {
      ...
      await service.storeCredential('test-namespace', 'expired', 'value1', false, 0.001);
      await service.storeCredential('test-namespace', 'active', 'value2', false, 60);
  ...
  describe('encryption', () => {
    it('should encrypt and decrypt correctly', async () => {
      ...
      for (const original of testValues) {
        await service.storeCredential('test-namespace', 'test-key', original, true);
        const retrieved = await service.getCredential('test-namespace', 'test-key');
    ...
    it('should use different encryption for same value', async () => {
      ...
      const file1 = await fs.readFile(path.join(testDir, 'namespace1.json'), 'utf-8');
      const file2 = await fs.readFile(path.join(testDir, 'namespace2.json'), 'utf-8');
