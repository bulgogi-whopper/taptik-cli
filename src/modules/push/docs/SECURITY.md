...
export class SecurityValidatorService {
  ...
  private readonly cmdPatterns = [
    /(\||;|&|\$\(|\`)/g,
    /(rm\s+-rf|chmod|chown|sudo)/gi,
...
const result = await db.query(
  'SELECT * FROM packages WHERE user_id = $1 AND name = $2',
  [userId, packageName]
);

// ❌ Bad: String concatenation
const result = await db.query(
  `SELECT * FROM packages WHERE user_id = '${userId}'`
);
...
export class GDPRComplianceService {
  ...
  async deleteUserData(userId: string): Promise<void> {
    ...
    await db.query(
      'UPDATE audit_logs SET user_id = NULL, metadata = metadata - \'email\' - \'name\' WHERE user_id = $1',
      [userId]
    );
