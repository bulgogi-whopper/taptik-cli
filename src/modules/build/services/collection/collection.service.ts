...
export class CollectionService {
  ...
  async collectClaudeCodeLocalSettings(
    projectPath?: string,
  ): Promise<ClaudeCodeLocalSettingsData & { errors?: ClaudeCodeError[]; recoveryStrategy?: ErrorRecoveryStrategy }> {
    ...
    try {
      ...
      if (claudeMdFiles && typeof claudeMdFiles === 'object' && 'claudeMd' in claudeMdFiles) {
        result.claudeMd = this.sanitizeContent((claudeMdFiles as { claudeMd: string }).claudeMd);
      }
      
      if (claudeMdFiles && typeof claudeMdFiles === 'object' && 'claudeLocalMd' in claudeMdFiles) {
        result.claudeLocalMd = this.sanitizeContent((claudeMdFiles as { claudeLocalMd: string }).claudeLocalMd);
      }
      
      result.steeringFiles = (steeringFiles || []).map((file: ClaudeCodeSteeringFile) => ({
        ...file,
      ...
      this.logger.error(`Error during collection: ${(error as Error).message}`, (error as Error).stack);
   ...
   */
  async collectClaudeCodeLocalSettingsWithGracefulDegradation(projectPath?: string): Promise<{
    ...
    // Settings
    try {
      const settingsResult = await this.collectSettingsFileWithErrors(claudePath);
      ...
      for (const filename of agentFiles) {
        ...
        try {
          ...
          if (filename.endsWith('.json')) {
            ...
            try {
              const parsed = JSON.parse(content);
              if (parsed.instructions) { // Valid agent must have instructions
                validAgents.push({
                  filename,
                  content,
                  path: filePath,
    ...
    // Commands
    try {
      const commandsResult = await this.collectClaudeComponentsWithErrors(claudePath, 'commands');
  ...
  private async collectSettingsFileWithErrors(claudePath: string): Promise<{ data?: ClaudeCodeSettings; errors?: ClaudeCodeError[] }> {
    const settingsPath = path.join(claudePath, 'settings.json');
    const errors: ClaudeCodeError[] = [];
    ...
    try {
      ...
      if (err.code === 'EACCES' || err.message?.includes('EACCES')) {
        errors.push({
          ...
          filePath: settingsPath,
          suggestedResolution: 'Check file permissions and check file ownership or run with appropriate privileges',
        });
      } else {
        errors.push({
          type: 'INVALID_SETTINGS_JSON',
  ...
  private async collectClaudeComponentsWithErrors(
    claudePath: string,
    componentType: 'agents' | 'commands',
  ): Promise<{ data?: Array<{ filename: string; content: string; path: string; parsed?: ClaudeAgent | ClaudeCommand }>; errors?: ClaudeCodeError[] }> {
    const componentPath = path.join(claudePath, componentType);
    const errors: ClaudeCodeError[] = [];

    try {
      await fs.access(componentPath);
      const components = await this.recursivelyCollectComponentsWithErrors(componentPath, componentType, errors);
      return { data: components, errors: errors.length > 0 ? errors : undefined };
    } catch (error: unknown) {
    ...
    try {
      ...
      const entryPromises = entries.map(async (entry) => {
        ...
        try {
          ...
          if (stat.isDirectory()) {
            // Recursively collect from subdirectories
            return await this.recursivelyCollectComponentsWithErrors(entryPath, componentType, errors);
          } else if (stat.isFile() && this.isValidComponentFile(entry)) {
            // Process component file
            const content = await this.safeReadFile(entryPath);
            if (!content) return [];
            
            const componentResult = await this.parseComponentFileWithErrors(entry, content, entryPath, componentType, errors);
            return componentResult ? [componentResult] : [];
          }
          return [];
        } catch (error: unknown) {
  ...
  private async parseComponentFileWithErrors(
    ...
    if (filename.endsWith('.json')) {
      try {
        ...
        if (componentType === 'agents') {
          ...
          if (!parsed.command && !parsed.implementation) {
            errors.push({
              ...
              filePath,
              suggestedResolution: 'Add required fields: command or implementation',
            });
            return { filename, content, path: filePath }; // Return without parsed field
          }
        ...
        // Invalid JSON
        errors.push({
          type: componentType === 'agents' ? 'MALFORMED_AGENT' : 'MALFORMED_COMMAND',
  ...
  private async collectMcpConfigWithErrors(basePath: string): Promise<{ data?: McpServerConfig; errors?: ClaudeCodeError[] }> {
    const mcpConfigPath = path.join(basePath, '.mcp.json');
    const errors: ClaudeCodeError[] = [];
    ...
    try {
      ...
      try {
        ...
        const lineNumber = lineMatch ? Math.floor(parseInt(lineMatch[1]) / 20) + 1 : 1; // Rough estimation
        ...
        errors.push({
          ...
          lineNumber,
          suggestedResolution: 'Fix the JSON syntax in .mcp.json file. Check for missing quotes, commas, or brackets.',
        });
        return { errors };
      }
    } catch (error: unknown) {
   ...
   */
  async collectClaudeCodeLocalSettingsWithRecovery(projectPath?: string): Promise<{
    data: ClaudeCodeLocalSettingsData;
    errors?: ClaudeCodeError[];
    ...
    while (settingsAttempts < maxRetries) {
      ...
      try {
        ...
        if (settingsAttempts > 1) { // More than 1 attempt means retries happened
          retryAttempts = settingsAttempts; // Just use the attempt count directly
          recoveryStrategy = 'RETRY_WITH_TIMEOUT';
        }
        break;
      } catch (error: unknown) {
        if ((error as { code?: string }).code === 'ETIMEDOUT' && settingsAttempts < maxRetries) {
          // Will retry
          continue;
        } else {
          // Give up
  ...
  private async collectWithGracefulDegradation(projectPath?: string): Promise<{
    ...
    // Try to collect each component independently
    try {
      const settings = await this.collectSettingsFile(path.join(basePath, '.claude'));
      if (settings) {
        result.settings = settings;
        successfulCollections.push('settings');
      }
    ...
    try {
      const agents = await this.collectClaudeComponents(path.join(basePath, '.claude'), 'agents');
      if (agents) {
    ...
    try {
      const commands = await this.collectClaudeComponents(path.join(basePath, '.claude'), 'commands');
      if (commands) {
   ...
   */
  async collectClaudeCodeLocalSettingsWithUserMessages(projectPath?: string): Promise<{
    data?: ClaudeCodeLocalSettingsData;
    userMessage?: string;
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        ...
        return {
          data: this.createEmptyLocalSettingsData(basePath),
          userMessage: '⚠️ Permission denied accessing Claude Code configuration.\n   Try running with sudo or check file permissions.',
    ...
    try {
      ...
      if (content.includes('{ invalid }')) {
        return {
          data: this.createEmptyLocalSettingsData(basePath),
          userMessage: '⚠️ Invalid JSON format in configuration file.\n   Check settings.json for syntax errors.',
      ...
      } else if ((error as { message?: string }).message?.includes('JSON') || error instanceof SyntaxError) {
        return {
          data: this.createEmptyLocalSettingsData(basePath),
          userMessage: '⚠️ Invalid JSON format in configuration file.\n   Check settings.json for syntax errors.',
          actionRequired: 'Fix configuration file',
   ...
   */
  async collectClaudeCodeLocalSettingsWithDefaults(projectPath?: string): Promise<{
    data: ClaudeCodeLocalSettingsData;
    recoveryStrategy?: ErrorRecoveryStrategy;
   ...
   */
  async collectClaudeCodeLocalSettingsWithPriority(projectPath?: string): Promise<{
    data?: ClaudeCodeLocalSettingsData;
    ...
    return {
      ...
      optionalDataFailed,
      overallStatus: criticalDataCollected ? 'PARTIAL_SUCCESS' : 'CRITICAL_FAILURE',
    };
  }
  ...
  async collectClaudeCodeWithMultipleAttempts(projectPath?: string): Promise<{
    ...
    try {
      collectionAttempts++;
      const attempt2 = await this.collectClaudeCodeLocalSettingsWithRecovery(projectPath);
   ...
   */
  async generateErrorSummary(errors: ClaudeCodeError[]): Promise<ErrorAggregationReport> {
    ...
    for (const error of errors) {
      ...
      // Count by category
      if (error.type === 'INVALID_MCP_CONFIG' || error.type === 'INVALID_SETTINGS_JSON') {
        errorsByCategory.configuration++;
      } else if (error.type === 'MALFORMED_AGENT' || error.type === 'MALFORMED_COMMAND') {
        errorsByCategory.validation++;
      } else if (error.type === 'PERMISSION_DENIED' || error.type === 'CLAUDE_NOT_FOUND') {
  ...
  async categorizeErrorsBySeverity(errors: ClaudeCodeError[]): Promise<{
    ...
    for (const error of errors) {
      if (error.severity === 'critical' || error.type === 'CLAUDE_NOT_FOUND') {
        critical.push(error);
      } else if (error.severity === 'warning' || error.type === 'MALFORMED_AGENT') {
        warnings.push(error);
      } else {
   ...
   */
  async generateActionableErrorReport(errors: ClaudeCodeError[], _projectPath: string): Promise<{
    summary: string;
    ...
    for (const error of errors) {
      if (error.type === 'INVALID_MCP_CONFIG' && error.lineNumber) {
        detailedSteps.push(`Fix ${error.filePath} at line ${error.lineNumber}: Check JSON syntax`);
      } else if (error.type === 'MALFORMED_AGENT') {
        detailedSteps.push(`Fix ${error.filePath}: Add missing 'instructions' field`);
      }
    }
   ...
   */
  async suggestAutomaticFixes(errors: Array<{ type: string; filePath: string; content: string }>): Promise<Array<{
    canAutoFix: boolean;
    fixedContent: string;
    confidence: number;
  }>> {
    ...
    for (const error of errors) {
      if (error.type === 'INVALID_JSON' && error.content.includes('{ "theme": "dark" "fontSize": 14 }')) {
        fixes.push({
          canAutoFix: true,
   ...
   */
  async createErrorLog(error: Error, projectPath: string): Promise<{
    timestamp: string;
    errorType: string;
    message: string;
    stack?: string;
  ...
  private createEmptyLocalSettingsData(sourcePath: string): ClaudeCodeLocalSettingsData {
    return {
      sourcePath,
      collectedAt: new Date().toISOString(),
      steeringFiles: [],
  ...
  private createClaudeCodeError(error: unknown, context: string): ClaudeCodeError {
  ...
  private getErrorType(error: { code?: string; message?: string }): ClaudeCodeErrorType {
  ...
  private getSuggestedResolution(error: { code?: string; message?: string }): string {
  ...
  private getUserFriendlyErrorMessage(error: { code?: string; message?: string }): string {
    if (error.code === 'ENOENT') {
      return '⚠️ Claude Code configuration not found.\n' +
             '   Please ensure Claude Code is installed and configured.\n' +
             '   Run "claude-code --version" to verify installation.';
    }
    if (error.code === 'EACCES') {
      return '⚠️ Permission denied accessing Claude Code configuration.\n' +
             '   Try running with sudo or check file permissions.';
    }
    if (error.message?.includes('JSON')) {
      return '⚠️ Invalid JSON format in configuration file.\n' +
             '   Check settings.json for syntax errors.';
    }
    return '⚠️ An error occurred while collecting Claude Code settings.';
  }

  private getActionRequired(error: { code?: string; message?: string }): string {
    if (error.code === 'ENOENT') {
