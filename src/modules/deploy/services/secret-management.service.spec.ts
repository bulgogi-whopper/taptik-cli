...
describe('SecretManagementService', () => {
  ...
  describe('detectSecrets', () => {
    ...
    it('should detect secrets in nested objects', async () => {
      const nestedConfig = {
        database: {
          connection: {
            ...
            connectionString: 'postgresql://user:verylongsecretpassword@localhost/db',
          },
        },
        auth: {
          jwt: {
      ...
      expect(secrets.length).toBeGreaterThan(0);
      expect(secrets.some(s => s.path === 'database.connection.password')).toBe(true);
    ...
    it('should handle arrays and complex structures', async () => {
      const complexConfig = {
        ...
        credentials: {
          providers: {
            aws: { accessKey: 'AKIA1234567890ABCDEF', secretKey: 'secret123456789012345678901234567890' },
          },
        },
      };
      ...
      expect(secrets.length).toBeGreaterThan(0);
      expect(secrets.some(s => s.value === 'key1234567890abcdefghijk')).toBe(true);
      expect(secrets.some(s => s.value === 'token1234567890abcdefghijk')).toBe(true);
    });
  ...
  describe('sanitizeConfiguration', () => {
    it('should replace detected secrets with placeholders', async () => {
      const configWithSecrets = {
        database: {
          ...
          connectionString: 'postgresql://user:verylongsecretpasswordstring@localhost/db',
        },
        apiKey: 'sk-1234567890abcdefghijklmnop',
      ...
      expect((sanitizedResult.database as Record<string, unknown>).password).toBe('${SECRET:database.password}');
    ...
    it('should generate secret mapping for restoration', async () => {
      ...
      const { sanitized, secretMapping } = await service.sanitizeConfigurationWithMapping(configWithSecrets);
      const sanitizedResult = sanitized as Record<string, any>;

      expect(sanitizedResult.token).toBe('${SECRET:token}');
      expect((sanitizedResult.nested as Record<string, unknown>).key).toBe('${SECRET:nested.key}');
      expect(secretMapping.length).toBeGreaterThan(0);
      expect(secretMapping.some(s => s.path === 'token' && s.value === 'mySecretToken1234567890')).toBe(true);
      expect(secretMapping.some(s => s.path === 'nested.key' && s.value === 'nestedSecret1234567890')).toBe(true);
    });
  });
  ...
  describe('error handling', () => {
    ...
    it('should handle environment variable injection errors', async () => {
      ...
      await expect(
        service.injectEnvironmentVariables(secrets)
  ...
  describe('private helper methods', () => {
    it('should calculate confidence correctly', () => {
      ...
      expect(calculateConfidence('apiKey', 'Abc123456789012345678901234567890')).toBeCloseTo(1);
    });
