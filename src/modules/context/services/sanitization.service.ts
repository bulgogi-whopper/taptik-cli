...
export class SanitizationService {
  ...
  sanitizeForCloudUpload(config: any): SanitizationResult {
    ...
    const securityLevel = this.determineSecurityLevel(severityBreakdown, findings);
    const recommendations = this.generateRecommendations(findings, severityBreakdown, detailedFindings);
  ...
  private sanitizeObject(
    ...
    if (typeof obj === 'string') {
      stats.totalFields++;
      result = this.sanitizeString(obj, path, findings, severityBreakdown, stats, detailedFindings);
  ...
  private sanitizeValue(
    ...
    if (typeof value === 'string') {
      return this.sanitizeString(value, key, findings, severityBreakdown, stats, detailedFindings);
    }

    if (typeof value === 'object') {
      return this.sanitizeObject(value, findings, severityBreakdown, stats, detailedFindings, path);
    }
  ...
  private sanitizeString(
    ...
    if (key.toLowerCase().includes('secret') || key.toLowerCase().includes('internal')) {
  ...
  private generateRecommendations(findings: string[], severityBreakdown: any, detailedFindings: any[]): string[] {
    ...
    const hasPrivateKeys = detailedFindings.some(f => f.category === 'privateKeys');
    const hasPasswords = detailedFindings.some(f => f.category === 'passwords');
    const hasTokens = detailedFindings.some(f => f.category === 'tokens');
    const hasConnectionStrings = detailedFindings.some(f => f.category === 'connectionStrings');
    ...
    if (hasPrivateKeys) {
      ...
      recommendations.push('Use secure key management systems for production deployments');
      recommendations.push('Consider using certificate-based authentication instead of embedded keys');
    }
    
    if (hasPasswords) {
      recommendations.push('Replace hardcoded passwords with secure credential storage');
      recommendations.push('Use OAuth or token-based authentication where possible');
    }
    
    if (hasTokens) {
      recommendations.push('Tokens should be stored securely and rotated regularly');
      recommendations.push('Use short-lived tokens with refresh mechanisms');
    }
    
    if (hasConnectionStrings) {
      recommendations.push('Store database credentials separately from connection strings');
      recommendations.push('Use connection pooling with encrypted credential storage');
    }
    
    if (hasAwsKeys) {
      recommendations.push('AWS credentials detected - use IAM roles instead of access keys when possible');
      recommendations.push('Enable AWS CloudTrail to monitor credential usage');
    }
    
    if (severityBreakdown.critical > 0) {
      recommendations.unshift('CRITICAL: Configuration contains highly sensitive data that must be removed before sharing');
    }
    
    if (severityBreakdown.medium > 2) {
      recommendations.push('Consider implementing a pre-commit hook to prevent sensitive data from being committed');
      recommendations.push('Use tools like git-secrets or detect-secrets for automated scanning');
    }
    
    // Add IP address recommendations
    if (detailedFindings.some(f => f.category === 'ipAddresses')) {
      recommendations.push('Replace hardcoded IP addresses with configuration variables');
    }
  ...
  private isCustomSecret(key: string, value: string): boolean {
    ...
    return secretKeyPatterns.some(pattern => pattern.test(key)) && 
           ...
           typeof value === 'string' && 
           value.length > 0 &&
           !this.isEnvironmentVariable(value); // Don't flag env vars as secrets
  }
  
  private isEnvironmentVariable(value: string): boolean {
    return /^\${[A-Z_][\dA-Z_]*}$/.test(value) || 
