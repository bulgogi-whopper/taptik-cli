...
describe('Cloud Platform Integration Tests', () => {
  ...
  const _createMockSanitizationResult = (
    overrides?: Partial<SanitizationResult>,
  ): SanitizationResult => ({
    sanitizedData: {},
    securityLevel: 'safe',
    findings: [],
  ...
  describe('Phase 13.1: .taptik Package Format Compatibility with Supabase Storage', () => {
    it('should create .taptik package with correct format for Supabase Storage', async () => {
      ...
      const metadata: CloudMetadata =
        await metadataGeneratorService.generateCloudMetadata(mockContext);

      const taptikPackage: TaptikPackage =
        await packageService.createTaptikPackage(metadata, mockContext, {});
    ...
    it('should compress package for efficient storage', async () => {
      ...
      const metadata: CloudMetadata =
        await metadataGeneratorService.generateCloudMetadata(largeContext);

      const taptikPackage: TaptikPackage =
        await packageService.createTaptikPackage(metadata, largeContext, {});
  ...
  describe('Phase 13.1: Cloud Metadata Schema Validation', () => {
    it('should generate metadata compliant with Supabase schema requirements', async () => {
        ...
        data: {
          claudeCode: {
            local: {
              ...
              mcpServers: {
                servers: [{ name: 'test-server', protocol: 'http' }],
              },
      ...
      expect(['claude-code', 'kiro-ide', 'cursor-ide']).toContain(
        metadata.sourceIde,
      );
  ...
  describe('Phase 13.1: Sanitization Effectiveness with Real-World Data', () => {
    it('should remove API keys and tokens from configuration', async () => {
      ...
      const sanitizationResult =
        await sanitizationService.sanitizeForCloudUpload(dataWithSecrets);
    ...
    it('should sanitize file paths and personal information', async () => {
      ...
      const sanitizationResult =
        await sanitizationService.sanitizeForCloudUpload(dataWithPaths);
      const sanitized = sanitizationResult.sanitizedData as any;

      // Verify path sanitization - service sanitizes paths
      expect(sanitized.personal.preferences.workspacePath).not.toContain(
        'johndoe',
      );
    ...
    it('should handle MCP server configurations securely', async () => {
      ...
      const sanitizationResult =
        await sanitizationService.sanitizeForCloudUpload(mcpData);
      const sanitized = sanitizationResult.sanitizedData as any;
      ...
      expect(sanitized.mcpServers.github.args).toEqual([
        '--from',
  ...
  describe('Phase 13.1: Package Integrity and Checksum Validation', () => {
    ...
    it('should validate package size limits for upload', async () => {
      ...
      const normalMetadata =
        await metadataGeneratorService.generateCloudMetadata(normalContext);
      const largeMetadata =
        await metadataGeneratorService.generateCloudMetadata(largeContext);
      ...
      const normalValidation =
        await validationService.validateForCloudUpload(normalPackage);
      const largeValidation =
        await validationService.validateForCloudUpload(largePackage);
  ...
  describe('Phase 13.1: Search Metadata Generation and Version Compatibility', () => {
    ...
    it('should validate version compatibility between source and target IDEs', async () => {
      ...
      const metadata =
        await metadataGeneratorService.generateCloudMetadata(claudeCodeContext);
      ...
      const validationResult =
        await validationService.validateForCloudUpload(taptikPackage);
      ...
      expect(validationResult.featureSupport.supported.length).toBeGreaterThan(
        0,
      );
      expect(validationResult.featureSupport.supported).toContain('themes');
      expect(validationResult.featureSupport.supported).toContain(
        'preferences',
      );
    ...
    it('should handle migration scenarios between different IDE versions', async () => {
      ...
      const oldMetadata =
        await metadataGeneratorService.generateCloudMetadata(oldVersionContext);
      const newMetadata =
        await metadataGeneratorService.generateCloudMetadata(newVersionContext);
      ...
      const validationResult =
        await validationService.validateForCloudUpload(oldPackage);

      // Debug: log validation errors if any
      if (!validationResult.isValid) {
        console.log(
          'Migration test - Validation errors:',
          validationResult.errors,
        );
        console.log(
          'Migration test - Validation warnings:',
